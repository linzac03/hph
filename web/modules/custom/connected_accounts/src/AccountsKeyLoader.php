...
class AccountsKeyLoader {
  public function getTwitterKey() {
    ...
    $tokens = $keyrepo->getKey('twitter_consumer_api_key');
    $tokens = $tokens ? $tokens->getKeyValue() : null;
  ...
  public function getFacebookKey() {
    $keyrepo = \Drupal::service('key.repository');
    $tokens = $keyrepo->getKey('facebook_api_key')->getKeyValue();
    if ($tokens == null) {
      drupal_set_message('Failed to load facebook','error');
      return [];
    }
